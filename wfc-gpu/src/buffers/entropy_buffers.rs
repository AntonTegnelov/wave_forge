...
mod tests {
    ...
    #[tokio::test]
    async fn test_entropy_buffers_creation() {
        let device = setup_device().await;
        let num_cells = 100;
        let config = DynamicBufferConfig::default();

        let buffers =
            EntropyBuffers::new(&device, num_cells, &config).expect("Failed to create buffers");

        assert_eq!(
            buffers.entropy_buf.size(),
            (num_cells * std::mem::size_of::<f32>()) as u64
        );
        assert_eq!(
        ...
        assert_eq!(
            buffers.staging_min_entropy_info_buf.size(),
            (2 * std::mem::size_of::<u32>()) as u64
        );
    }

    #[tokio::test]
    async fn test_entropy_buffers_resize() {
        let device = setup_device().await;
        let initial_cells = 100;
        let config = DynamicBufferConfig::default();

        let mut buffers =
            EntropyBuffers::new(&device, initial_cells, &config).expect("Failed to create buffers");
        let new_cells = 200;
