#!/usr/bin/env bash
# Renders a pack's ground with a material per category and saves a picture, to look at (developer
# tool).
#
...
exec "${GODOT:-godot}" --rendering-driver opengl3 --path "$here/godot" --script render_ground.gd
