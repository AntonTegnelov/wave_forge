...
const PACK: &str = r#"(
    ...
    stages: [
        ...
        (name: "shore", kind: Blur(input: "height", radius: 2)),
    ],
)"#;
...
fn a_pack_reports_how_far_each_stage_is_generated_beyond_the_target() {
    ...
    let reach = pack.reach("shore").expect("a stage");

    assert_eq!(reach["shore"], 0);
    assert_eq!(reach["height"], 2);
    assert_eq!(reach["smooth"], 2);
    assert_eq!(reach["rough"], 7);
    assert_eq!(reach["detail"], 7);
}
