...
|---|---|---|---|---|
| **Field** | a fused pointwise expression graph: noise, splines, remap, math, first-match classifiers, image lookup | 0 | G1, G3, G6, G7, N2, N7 | value noise with named streams, coordinates and distances, arithmetic, clamps, smoothsteps, remaps, curves, selects, a blended match per category, and Godot's FastNoiseLite noises in 2D, exact against Godot |
| **Filter** | stencils, blur, cellular automata, slope, distance transforms | declared per pass | G2, G5, G7 | `Blur`, `Delta` (terrain delta), `Area` (biome area) and `Flatten`; slope, cellular automata and distance transforms are not built yet |
...
(height, slope, any Rules-style condition, water depth, a margin from sites, a clearance from other
Scatter stages' points, which are placed first and so take priority), spacing, and a scale, a tilt
and a stance along the ground per point ([packs.md](../reference/packs.md#scatter)). The asset
binding is **not built yet** ([#44](https://github.com/AntonTegnelov/wave_forge/issues/44)).
