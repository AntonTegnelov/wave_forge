...
The world's answer to a status is a **repair**: solve the chunk alone with its halo released, which lets it rewrite the neighbouring cells the halo covers, and widen the halo if that is not enough. A repair solves the region once per seed of `RepairPolicy::seeds` (32 by default) in one dispatch and keeps the lowest seed that solved, so the outcome depends on nothing but the configuration. A seed stops as soon as a lower one has solved, since it can no longer be the one kept, so a repair costs about as long as the seeds up to its winner. A chunk that still will not solve is reported once and left alone, because solving it again would fail the same way.
...
- A repair rewrites cells of the neighbours its halo covers, and the world stays a function of its configuration. A repair waits until every chunk it can see has had its first attempt (the scheduler generates any that nobody asked for) and until the failed chunks of lower **repair classes** around it are repaired. A chunk's class is the parity of each of its coordinates, so two chunks within one of each other, diagonals included, are in different classes, and repairs of one class are at least two chunks apart; with a repair halo short of half a chunk (at most 3 cells for chunks of 8) they neither read nor write each other's cells. What a repair sees is then the same whatever order the world was generated in, and so is what it writes: a 4×4-chunk city with repairs comes out tile for tile the same generated all at once or chunk by chunk in either raster order (`wfc-devtools/tests/order_diff.rs`, seeds 8 and 11). Every chunk a repair rewrote is reported as an `Updated` event and counted.
- Two limits remain. In a world more than one chunk tall, a first-parity repair also reaches the corner chunks of the other parity, which it does not wait for, because they may be waiting for it. And a chunk evicted and generated again comes back as its first attempt, without the repairs of neighbours that had rewritten it, so evicting part of a repaired neighbourhood can leave a seam. A rule set is **streaming-clean** when no repair is ever needed; the city is not (about one chunk in nine is repaired).

### 6.4 Streaming

`WorldGenerator::request` takes focus points (a chunk and a radius), and `tick` starts the next batch: the nearest eligible chunks of one parity, then, if there is nothing left to start, the lowest-class repair whose neighbourhood is complete (§6.3). `poll` commits whatever has finished and returns events. `evict_outside` hands far chunks back so a game can persist them, and `import` puts one back.
