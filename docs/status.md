...
- **Contradictions backtrack** (A-9): the run jumps back to the most recent choice next to where the failure surfaced (or undoes a doubling number of steps when there is none) and forbids the choice it returns to. There is still no seeded restart, and the history is capped.
