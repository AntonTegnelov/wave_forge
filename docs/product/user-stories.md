...
[#104](https://github.com/AntonTegnelov/wave_forge/issues/104). **Status:** verified by automated checks (2026-09-25), in the library's CI job and, for edits, saves and pieces in Godot, the Godot job of every pull request's CI. The ring world (`examples/rings.world.ron`) is a bounded disk island with biomes by distance and noise, per-biome height, lakes in its hollows and rivers that run to the sea or a lake, carved into its ground, a location table of shrines and crypts per ring with quotas and spacing, ore and birch groves placed by rules, and a dungeon of prefab rooms grown above every crypt, rerolled until it has eight pieces, each the same in any order (`tests/bound.rs`, `tests/rules.rs`, `tests/blend.rs`, `tests/rivers.rs`, `tests/lakes.rs`, `tests/locations.rs`, `tests/scatter_chain.rs`, `tests/assemble.rs`). Placements have stable ids, and felling one or editing terrain persists across eviction and regeneration (`tests/edits.rs`, `verify_edits.gd`). Its grass is clutter, regenerated and never saved, and its locations are frozen in a save (`tests/save.rs`, `verify_edits.gd`). The valley pack adds towns on levelled sites, served to a game by a Godot node and a Bevy plugin with ground a walker crosses ([testing.md](../guides/testing.md)). Valheim's features beyond the criteria, paths between sites, lakes and scenes bound to points, are in [story-coverage.md](../plan/story-coverage.md#g7-a-valheim-like-island-world).
