...
| E37 | The city check with at most 3 collider bodies per frame, ten runs | the node's own process max 2.4 to 3.7 ms, p99 0.59 to 0.67 ms, where one full run had reached 8.8 ms building 9 chunks' bodies; nine runs passed. The tenth failed on Godot's slowest frame, 8.5 ms, while the node's slowest was 3.1 ms (3 bodies 1.4 ms, navigation 1.7 ms), with one frame period of 24.7 ms: a stall outside the node, of the kind E19 recorded once at 12.2 ms | the city check at [#118] |
| E38 | The stages check with bodies bounded too, ten runs | every run passed; the node's own process p99 0.38 to 0.45 ms, max 1.6 to 2.6 ms | the stages check at [#118] |
| E39 | The history example's first village, from asking to `stage_ready`, three runs | at a view of 5, its ground 1.69 and 1.77 s and its town 5.54 and 5.64 s, with 2 316 and 2 240 products generated by then; at a view of 3, 1.67 s and 5.58 s. The town's first solve dominates, as E34 found | a headless script over `examples/history` (seed 7, kernels cached, targets `level` and `towns`), release, dozen on the RTX 3070 |
| E40 | The same with products ordered nearest first across stages, not stage by stage | the ground 5.47 to 5.62 s and the town at the same moment, with 488 to 944 products by then: the town solve ran in the step that held the ground and held back its report. Stage by stage stayed ([#126]) | the same script on a branch that was not merged |
| E41 | Compiling a town's kernels at once, each pipeline on a thread of its own, release | warming for 2×2 and 1×3 towns, 6 pipelines in 8 450 ms on the clock against 31 217 ms one after another; a first town cold 8 308 ms (8 003 compiling 3 pipelines), from a pipeline cache 3 174 ms (2 909 compiling), against E34's 8 556 and 3 815 ms: compiles contend on this driver | `wfc-devtools/tests/town_kernels.rs`, dozen on the RTX 3070, at [#111] |
| E42 | Towns solved on a thread of their own, each compiling its kernels at once, three runs | the valley's first town 8.0 to 8.2 s after it was asked for cold and 3.9 to 4.0 s with cached kernels (E34: 8.5 to 8.7 and 4.8 s), counting only a chunk that holds a town; the history example's villages 5.0 to 5.2 and 5.8 to 6.1 s (E39: 5.54 to 5.64 s). Warming every size the pack allows from the start instead held the history example's villages to 17.6 and 22.3 s, compiling sizes they did not need ([#146]) | `verify.sh release`, dozen on the RTX 3070, at [#111] |
...
| E47 | Ground levels of detail: 225 chunks of 8×8 cells (the ground check's pack), seen from 40 units up at 65° over a background any gap would show; primitives drawn and gap pixels per viewport `mesh_lod_threshold` | levels off 10 944 primitives; 1 pixel (Godot's default) 8 472; 4 pixels 1 624; 16 pixels 570, every chunk at its coarsest. No gap at any threshold; with the skirts at zero depth, 121 gap pixels at 1 pixel and 1 600 at 4 | `render_lods.gd` under `xvfb-run`, Compatibility (OpenGL) over dozen on the RTX 3070, release, at [#46] |
| E48 | Starting a navigation bake in the city check, split into its parts: a probe over ten runs (390 bakes), then ten runs with `navigation_source_ms` in `stats()` | the probe: gathering the source triangles 0.38 ms on average and 1.0 at most (13 824 triangles at most), handing them to Godot (the source resource, ~124 000 floats copied) 0.21 and 3.16, the asynchronous bake call 0.03 and 0.1. So a start's variance is in the handover, the allocating and copying a host stall slows, which fits E46's run, whose bakes on the navigation server's threads were slow too. The ten runs all passed: Godot's slowest frame 2.4 to 3.9 ms, the node's navigation in it at most 2.2 ms, a start at most 2.1 ms of which the source 1.55. E46's 8.9 ms start did not recur in these twenty runs ([#157]) | the city check, `verify.gd` in release, dozen on the RTX 3070 |
| E49 | The city check, the first run after the host restarted, then again | the first failed the 8 ms bar at 8.7 ms, of which the node's own work was 3.1 ms (navigation 2.2, colliders 0.8, a start's source within E48's range): 5.6 ms of that frame were Godot's thread outside the node, a host stall of E37's kind rather than the node's. The run straight after passed at 2.5 ms | the city check, `verify.gd` in release, dozen on the RTX 3070, 2026-09-25 |
| E50 | Occluders in the city (`examples/city.ron`, seed 11, chunks of 8³ cells of 2 units, 7 by 7 chunks drawn as one MultiMesh per module and chunk, occluders on the 5 by 5 around the camera), camera still, vsync off, 400 frames a phase, three runs a view: occlusion culling off; on with the occluders standing; on while three chunks' occluders are freed and built again every frame, the node's budget. Godot's CPU time for the viewport (`viewport_get_measured_render_time_cpu`), p50 | from above the roofs: objects drawn 1 319, 1 308 and 1 305, so under 1% culled, a chunk's MultiMesh being culled only whole; viewport CPU off 21.1 to 23.3 ms, on 23.4 to 24.8, churn 22.0 to 27.3, differences within the 2 to 4 ms the runs vary by. From a street at eye height: objects drawn 711, 329 and 346, 54% culled; viewport CPU off 14.4 to 15.5 ms, on 9.4 to 13.4, churn 12.3 to 15.0: culling saves 2 to 6 ms and rebuilding three chunks' occluders a frame costs 1.5 to 3 ms of it back. A runner at 4.2 units/s crosses a chunk every 4 s and rebuilds a ring of 5 over two frames | `render_occlusion.sh` under `xvfb-run`, Compatibility (OpenGL) over dozen on the RTX 3070, release, at [#47] |
| E51 | Placing a village's pieces again with a pooled scene against a fresh one: 46 houses and streets bound to a `Node3D` with a mesh child and a script, the 22 outside a promotion radius freed and placed again in one process call, five cycles a run, three runs; the node's process timed once with nothing to place and once placing them, the difference per node, `instance_spawned` and the check's handler included | pooled (the script defines `_wave_forge_reset`): medians 8.0 to 10.7 µs a node, runs from 7.1 to 12.3; fresh (the same script without it): medians 12.4 to 13.8 µs, from 10.6 to 16.3. Reuse saves 3 to 5 µs a node end to end, far less than E44's 25 times, since moving, attaching, the signal and its handler cost the same either way. Headless, so the dummy renderer | `verify_pooling.gd` in headless Godot 4.7.2, release, at [#44] |
...
[#183]: https://github.com/AntonTegnelov/wave_forge/issues/183
