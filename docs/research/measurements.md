...
| Live streaming through the library facade, 24×8 chunks, view radius 4 | 186 chunks (95 232 cells) in **1.05 s of dispatches**: median tick **47 ms**, p90 61 ms, busiest 322 ms (the initial 44-chunk fill) inside a 500 ms budget, 86k cells/s while generating; 19 chunks repaired, 107 chunks rewritten by those repairs, 0 seam violations | `wfc-devtools/tests/streaming.rs::live_streaming_keeps_ahead_of_a_walking_player`, release, dozen on an RTX 3070, world seed 11, halo 1, radius 1 with undo, kernels compiled before timing |
| Same, chunks no repair could place | **6 of 186 (3.2%)**, against 1.6% for the same world driven by the bench | the facade closes the wanted set over face neighbours, so every chunk of the second parity is solved against four fixed faces where the bench's frontier often left one or two free: tiles that do not depend on where the player came from cost placements |
...
| Repairs stop every seed above one that has solved, live streaming, 24×8 chunks | 20 repairs in **255 ms** instead of 600 (12.8 ms each instead of 30); median tick **40 ms**, p90 **69 ms**, busiest 167 to 433 ms over three runs; 0 of 192 could not be placed | `streaming.rs::live_streaming_keeps_ahead_of_a_walking_player`, same build and stack; a seed can only be stopped by a lower seed that solved, so the winner is the one every seed running to its end would give |
| Same, five census worlds | the same repairs (30, 35, 31, 36, 44) rewriting the same number of chunks as without stopping, in **3.6 s** of solver time instead of 6.5 s; 0 of 1 605 given up on | `hole_census.rs`, seeds 11, 23, 47, 101, 977 |
| Same, game session | 0 holes in view, 0 of 455 given up on, 45 repairs in 1.75 s of 9.97 s in the solver, 0 chunk-frames late, main thread p99 0.004 ms | `game_session.rs` |
| Repairs that wait until every chunk they can see has had its first attempt, lowest repair class first (docs/architecture.md §6.3) | a 4×4-chunk city generated all at once, chunk by chunk in raster order and in reverse raster order is **tile for tile the same**, seeds 8 and 11, repairs included; before, 12 and 6 of the 16 chunks differed between orders | `order_diff.rs`, dozen on an RTX 3070 |
| Same, five census worlds | **0 of 1 772** given up on (more chunks than before, because repairs have neighbours generated that no focus asked for); 181 repairs against 176; **4.0 s** in the solver against 3.6 s | `hole_census.rs`, release, same stack, seeds 11, 23, 47, 101, 977 |
| Same, game session | 0 holes in view, 0 given up on, 58 repairs, 0 chunk-frames late, main thread p99 0.005 ms, 9.1 s in the solver; at most 94 chunks held; 42 chunks walked back to, all identical; 0 violations across 2 454 seam checks | `game_session.rs`, release, same stack |
| Same, live streaming, 24×8 chunks, view radius 4 | 0 of 192 could not be placed, 24 repairs in 347 ms; median tick **53 to 54 ms** (was 40), p90 **128 to 130 ms** (was 69), every tick of play inside the 500 ms budget, the initial 44-chunk fill 493 ms; the price of order independence is repairs that wait for their neighbourhoods, so more of the work lands in fewer ticks | `streaming.rs::live_streaming_keeps_ahead_of_a_walking_player`, release, same stack, three runs |
