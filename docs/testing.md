...
| Order independence | `wfc-devtools/tests/order_diff.rs` | The facade's order contract on a real device with the city: a 4×4-chunk city generated all at once, chunk by chunk in raster order and in reverse order is the same world, tile for tile, with the same chunks given up on, when repairs are off; a mismatch names how many chunks differ and the first differing cell. With repairs on it measures the difference instead (`-- --ignored`), because repairs rewrite solved neighbours ([#66](https://github.com/AntonTegnelov/wave_forge/issues/66)); 12 of 16 chunks differed for seed 8 and 6 of 16 for seed 11 (dozen on an RTX 3070) | Yes |
