...
| Godot extension | `wave_forge_godot/godot/verify.gd` | The city's tiles are named, turned and tagged the way a game needs to place them, and a focus runs a strip of chunks and back inside a real Godot, by the clock and without waiting for generation: the chunks beside it are there on every frame, chunks behind are dropped, tiles obey the rules across seams, a chunk returned to is unchanged, Godot's slowest frame stays under 8 ms, and the node's own time per frame under 2 ms at the 99th percentile | Yes, and a Godot binary |
