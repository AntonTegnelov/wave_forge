...
`verify_stages.gd`, `verify_tables.gd`, `verify_noise.gd`, `verify_edits.gd`, `verify_assemble.gd`, `verify_scenes.gd`, `verify_ground.gd` and `verify_sound.gd` headless, prepares `examples/history`
and runs its `check.gd`. Each exits non-zero on any failure and prints what it generated and how
