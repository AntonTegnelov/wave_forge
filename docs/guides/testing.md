...
`verify_stages.gd` and `verify_tables.gd` headless, prepares `examples/history` and runs its
`check.gd`. Each exits non-zero on any failure and prints what it generated and how
