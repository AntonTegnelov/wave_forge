...
| Godot ground materials | `wave_forge_godot/godot/verify_ground.gd` | Every chunk's ground is drawn with a copy of the reference ground shader's material of its own, whose id texture holds the category of every ground vertex, the chunks beyond its far edges included, and whose cell is the node's; a material stage that is no Rules or Area stage is refused; grass grows on every chunk of ground within its radius and none beyond, each chunk's grass material holding its cover per column and the ground's height per vertex; `ground_height` is the column's height at every column centre and on each square's diagonal halfway across | Yes, and a Godot binary |
