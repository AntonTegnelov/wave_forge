...
| Godot ground materials | `wave_forge_godot/godot/verify_ground.gd` | Every chunk's ground is drawn with a copy of the reference ground shader's material of its own, whose id texture holds the category of every ground vertex, the chunks beyond its far edges included, and whose cell is the node's; a material stage that is no Rules or Area stage is refused | Yes, and a Godot binary |
