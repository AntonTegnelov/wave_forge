...
| Godot scenes | `wave_forge_godot/godot/verify_scenes.gd` | Scenes bound to a village's pieces and to trees: a lone-mesh tree is drawn once per point as MultiMeshes; houses given as a `PackedScene` and streets given by a path loaded on the loader threads get exactly one node per piece, named by `instance_spawned`, at the piece's transform; moving away frees every node and MultiMesh; placing's slowest frame is printed | Yes, and a Godot binary |
