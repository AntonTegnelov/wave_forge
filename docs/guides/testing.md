...
`verify_assemble.gd`, `verify_scenes.gd`, `verify_ground.gd`, `verify_sound.gd` and
`verify_names.gd` headless, prepares `examples/history` and runs its `check.gd`. Each exits non-zero on any failure and prints what it generated and how
