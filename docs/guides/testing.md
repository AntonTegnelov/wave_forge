...
| Saves | `tests/save.rs` | A frozen stage keeps the chunks it first generated after the pack changes, and a chunk never generated before follows the new pack; an ephemeral stage's edits are never saved; a save records the generator's version and the pack's digest, and loads as it was written; the ring world freezes its shrines and never saves its grass | No |
