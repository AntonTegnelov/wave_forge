...
`verify_names.gd` and `verify_occlusion.gd` headless, prepares `examples/history` and runs its `check.gd`. Each exits non-zero on any failure and prints what it generated and how
