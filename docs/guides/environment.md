...
- `verify.sh` runs `prepare.sh`, then `verify.gd`, `verify_stages.gd`, `verify_tables.gd`,
  `verify_noise.gd`, `verify_edits.gd`, `verify_assemble.gd`, `verify_scenes.gd` and
  `verify_ground.gd` headless, then the history example's `prepare.sh` and `check.gd`. Use `release`:
