...
2. **Tables of facts** ([#72](https://github.com/AntonTegnelov/wave_forge/issues/72)): rows the game
   ...
   engines' access, sites from a table's rows and towns by a row's names are built; next are
   roads with [#98](https://github.com/AntonTegnelov/wave_forge/issues/98), the example, and regenerating only the chunks a changed row
   touches. With region jobs
