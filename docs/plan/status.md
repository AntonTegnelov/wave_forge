...
- **The stage runtime is a first slice.** Fields are one `f32` per column, at a scale per stage;
  ...
  roads
  ([#72](https://github.com/AntonTegnelov/wave_forge/issues/72)); there are no edits or persistence modes; noise is the library's value noise and Godot's FastNoiseLite in 2D; a Scatter stage has one kind. Each is an issue under
