...
| Rivers and streams that carve the ground | curves, a network between sites, rasterising curves into height | available: a Rivers stage runs rivers downhill to the sea in every region, and Apply carves them into the ground without seams (`tests/rivers.rs`, `tests/apply.rs`); a Network stage finds paths between sites (`tests/network.rs`); a river stops at its region's edge, and rivers do not join lakes | [#144](https://github.com/AntonTegnelov/wave_forge/issues/144) |
...
| The ground's mesh and collider | a ground product, seamless across chunks, with material ids | available: a mesh and a height-field collider per chunk in Godot, with a material per category of a Rules stage through a reference ground shader (`verify_ground.gd`); a mesh and height grid in Bevy, whose materials are to come | [#46](https://github.com/AntonTegnelov/wave_forge/issues/46) (Bevy's materials) |
