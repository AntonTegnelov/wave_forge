...
pub async fn solve(
    ...
    solve_rules(initial, &fixture.rules, None, None, boundary, attempts)
        .await
        .grid
}
...
pub async fn solve_rules(
    ...
    solve_rules_with(
        initial, rules, weights, constraint, None, boundary, attempts,
    )
    .await
}
...
pub async fn solve_rules_with(
    ...
    for attempt in 1..=attempts {
        ...
        if let Some(weights) = weights {
            accelerator
                .with_tile_weights(weights)
                .expect("valid tile weights");
        }
        ...
        if let Some(batch) = std::env::var("WFC_COLLAPSE_BATCH")
            .ok()
            ...
            .await
        {
            Ok(grid) => {
                return Solved {
                    grid,
                    attempts: attempt,
                    solve_time: run_started.elapsed(),
                    total_time: started.elapsed(),
                };
            }
            Err(error) if error.to_string().contains("Contradiction") => {
                eprintln!(
                    "attempt {attempt}/{attempts} hit a contradiction after {:?}: {error}",
                    run_started.elapsed()
                );
